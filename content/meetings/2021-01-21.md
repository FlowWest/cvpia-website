...
We’ve established the Near-term Restoration Strategy priority restoration actions and info needs
Next is to proptype the model changes based on the proposals reviewed and accepted by the SIT
SIT reviews the results of prototyping and affirm the changes to the model (is the model improved?)
Then the next round of model calibration and sensitivity analysis will occur (does the model still make sense?)
Leads to a new release of the model
We will run the existing priorities through the updated model to obtain updated output
SIT assesses the revised model output (are existing priorities still supported?)
Based on the revised output, SIT determines whether revised prioritization is required. If so, SIT will follow established process to develop additional candidate strategies and update the priorities list (which will be documented in the Adaptive Management Update Annual Memo)
